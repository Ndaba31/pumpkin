...
const ProfileSegment = ({ profile }) => {
	...
	return (
		<div className={styles.segmentGrid}>
			{profile.dob !== undefined && profile.dob !== null && (
				<div className={styles.segmentContainer}>
					<h4>Date of Birth:</h4>
					<button className={styles.pills}>{birth}</button>
				</div>
			)}
			{profile.relationship_status !== undefined && profile.relationship_status !== null && (
				<div className={styles.segmentContainer}>
					<h4>Relationship Status:</h4>
					<button className={styles.pills}>{profile.relationship_status}</button>
				</div>
			)}
			{profile.religion !== undefined && profile.religion !== null && (
				<div className={styles.segmentContainer}>
					<h4>Religion:</h4>
					<button className={styles.pills}>{profile.religion}</button>
				</div>
			)}
			{profile.sex !== undefined && profile.sex !== null && (
				<div className={styles.segmentContainer}>
					<h4>Sex:</h4>
					<button className={styles.pills}>{profile.sex}</button>
				</div>
			)}
			{profile.ethnicity !== undefined && profile.ethnicity !== null && (
				<div className={styles.segmentContainer}>
					<h4>Ethnicity:</h4>
